...
ui <- shiny::fluidPage(
    ...
    shiny::tabsetPanel(id = "mainTabset",
        shiny::tabPanel(title = "Load Dataset", value = "dataset_tab",
            shiny::sidebarLayout(
                sidebarPanel = shiny::sidebarPanel(width = 3,
                    shiny::fileInput("file1", "Choose Data File", multiple = FALSE, accept = c("text/csv", "text/comma-separated-values,text/plain", ".csv", ".xls", ".xlsx", ".ods")),
                ),

                mainPanel = shiny::mainPanel(
                    DT::dataTableOutput(outputId = "datasetTable")
                )
            )
        ),
        shiny::tabPanel(title = "Match New Data", value = "new_data_tab",
            shiny::h3("Load single or multiple"),
            # Sidebar panel, similat to main data
            # Maybe second tabset panel with multiple matches... lets see
        ),
        shiny::tabPanel(title = "Handle Multiple Matches And Unclassified Samples", value = "handle_multiple_matches_and_unclassified_samples",
            ...
            shiny::conditionalPanel("input.multipleMatchIndex != ''",
                ...
                shiny::h4(shiny::textOutput("multiMatchDataFor")),
                ...
                shiny::sidebarLayout(
                    sidebarPanel = shiny::sidebarPanel(width = 9,
                        # Render the samples that matched/were close
                    ...
                    mainPanel = shiny::mainPanel(
                        ...
                        leaflet::leafletOutput(outputId = "multiMatchMap"),
                    ),
                ),
                # User choose and add to a group of samples/individual - information
                shiny::h5("If this ID is one of the listed above the sample will be added to that group of sample/individual, if not, the sample will create a new individual IF the new ID does not already exist, make sure it is unique if that is the desired action."),
        ...
        shiny::tabPanel(title = "Test New Data", value = "test_new_data",
            ...
            shiny::radioButtons(inputId = "new_data_mode", label = "Add/Test new data by: ", choices = c(Single = "single", Multiple = "multiple"), selected = "single"),
            ...
            shiny::conditionalPanel(condition = "input.new_data_mode == 'multiple'",
                ...
                shiny::fileInput(inputId = "new_data_file", label = "Choose Data File", multiple = FALSE, accept = c("text/csv", "text/comma-separated-values,text/plain", ".csv", ".xls", ".xlsx", ".ods")),
            ),
            shiny::tags$hr(),
